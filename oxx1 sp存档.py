...
class DQNAgent:
    ...
    def select_action(self, state):
        if np.random.uniform() < self.epsilon:
            return torch.tensor([[np.random.randint(self.output_dim)]])
        else:
            with torch.no_grad():
                return torch.tensor(self.policy_net(state).squeeze(1).data.max(1)[1].item())
    ...
    def train(self):
        ...
        next_state_values = self.target_net(next_states).gather(1, next_action_batch)
...
def train(num_episodes):
    ...
    for episode in range(1,num_episodes+1):
        ...
        while not done:
            ...
            next_state, reward, done, truncated, _ = env.step(action.item())  # 将动作转换为普通的Python整数
