...
class ReplayMemory_Per(object):
    ...
    def __init__(self, n_step,capacity, blank_trans,n_history,a=0.6, e=0.01,b=-0.4 ):
    ...
    def sample(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            for step in range(self.n_step):
                if (dataIdx + step)<self.tree.n_entries:
                    ...
                    break
            # 获取过去N步经验
            sequence_history_datas = []
            for step1 in range(self.n_history, -1, -1):
                if (dataIdx - step1) >= 0:
                    # 获取历史经验
                    sequence_history_datas.append(self.tree.getdata(dataIdx - step1))
                else:
                    sequence_history_datas.append(self.blank_trans)
            history_datas.append(sequence_history_datas)
        ...
        return idxs,pts, sample_datas,history_datas   #返回batch_size个叶子号，数据值
...
class DQNAgent:
    def __init__(self, input_dim=64, output_dim=4, gamma=0.9, epsilon=1, epsilon_decay=0.9998, min_epsilon=0.01,
        ...
        self.n_step=n_step
        self.n_history=n_history
        self.blank_trans = [np.zeros((self.input_dim), dtype=np.uint8), -5, 0.0, np.zeros((self.input_dim), dtype=np.uint8),0] #action 0有问题
        # 创建经验回放缓冲区
        self.buffer = ReplayMemory_Per(capacity=buffer_capacity, n_step=n_step, n_history=n_history,
    ...
    def zip_transitions(self,transitions):
        states = []
        actions = []
        rewards = []
        next_states = []
    ...
    def train(self):
        if self.buffer.size() < self.batch_size:
            return
        idxs, pts, transitions ,history_transitions= self.buffer.sample(self.batch_size)  #返回叶子号、数据值
        if np.isnan(self.buffer.tree.total()):
            print('error')   #树优先值溢出
        _, _, rewards, next_states, dones =self.zip_transitions(transitions)  #未来奖励，状态，done
        states = []
        actions = []
        for idx,batch in  enumerate(history_transitions):
            # 提取states全部状态
            states123 = []
            states123.append(batch[0][0])
            # 获取最后一个奖励、下一个状态和完成标志
            current_actions = batch[0][1]
            reward_sum = batch[0][2]
            current_next_state = batch[0][3]
            current_done = batch[0][4]
            for i, (state, action, reward, next_state, done) in enumerate(batch):
                ...
                if done:
                    # 如果序列长度不足，添加最后一个状态
                    while len(states123) < self.n_history + 1:
                        states123.append(states123[-1])
                    rewards[idx]=reward_sum
                    next_states[idx]=current_next_state
                    dones[idx]=current_done
                    break
            states.append(states123)
        ...
        next_states=next_states.repeat(1, 4, 1)
...
def train(num_episodes):
    ...
    for episode in range(1,num_episodes+1):
        ...
        while not done:
            ...
            stateact = list(state_queue)
            while len(stateact) < agent.n_history + 1:
                stateact.append(stateact[-1])
            action = agent.select_action(FT([[stateact]]))
            next_state, reward, done, truncated, _ = env.step(action)  # 将动作转换为普通的Python整数
            ...
            if done:
                ...
                reward =-0.001
            agent.store_transition((state, action, reward, next_state,done))  # 同样将动作转换为Python整数
            state = next_state
            if up==100:
                break
    ...
    # 保存模型
    torch.save(agent.policy_net.state_dict(), "frozen_lake_dqn_model-pri.pth")
    env.close()
...
def trainagain(num_episodes):
    ...
    agent.policy_net.load_state_dict(torch.load('frozen_lake_dqn_model-pri.pth'))
    total_reward = 0
    for episode in range(1,1+num_episodes):
        ...
        while not done:
            up += 1
            action = agent.select_action(FT([[state]]))
            next_state, reward, done, _ = env.step(action.item())  # 将动作转换为普通的Python整数
            total_reward += reward
            if done:
                if reward > 0:
                    reward = reward + 999
